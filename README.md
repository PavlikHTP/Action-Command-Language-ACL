...
Он позволяет маперам и ивент-мейкерам динамически вставлять значения об игроках, случайных числах, времени и других переменных прямо в текст команд.
...
```
give {PlayerId} item_medkit
broadcast 5 Welcome, {PlayerName}!
```

Перед выполнением, движок заменит плейсхолдеры на их реальные значения.
...
| `{PlayerName}` | `JohnDoe` | Ник игрока |
| `{SteamId}` | `76561198000000000` | Steam ID игрока |
| `{DisplayName}` | `Player #1` | Отображаемое имя игрока |
